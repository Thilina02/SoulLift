...
import { StarIcon } from "@chakra-ui/icons"; // Assuming you have the Chakra UI Icons imported
...
const ProductActions = ({ product }) => {
  ...
  const [rating, setRating] = useState(0); // State for rating, initially set to 0
  ...
  const handleReview = async () => {
	...
	try {
	  const res = await fetch("/api/products/review/" + product._id, {
		...
		body: JSON.stringify({ text: review, rating }), // Corrected "rating"
	  });
	  const data = await res.json();
	  if (data.error) return showToast("Error", data.error, "error");
  ...
  const handleStarClick = (selectedRating) => {
    if (isReviewing) return; // Prevent rating change while posting a review
    ...
    <Flex flexDirection="column">
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <StarIcon
            key={index}
            color={rating > index ? "yellow.400" : "gray.200"}
